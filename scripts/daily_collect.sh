...
# 스냅샷과 수집은 **각자 프로세스라 토큰을 각자 받는다.** 1초 안에 두 번 받으면
# 발급 엔드포인트(au10001)의 유량 제한에 걸린다 — 2026-09-01 에 그래서 첫 종목
# (005930)이 유실됐다. 클라이언트가 재시도로 막지만(client.py: _fetch_token),
# 애초에 붙여 부르지 않는 편이 낫다. 3분짜리 작업에서 5초는 공짜다.
sleep 5
...
# 수급(투자자별 순매수). **증분이라 하루치는 종목당 1페이지**로 끝난다 —
# README 의 "종목당 30~50초"는 2,865행짜리 최초 백필 비용이고, 갱신은 1초 미만이다.
# 실측(2026-09-03): 005930 신규 8행에 1초 미만, 최초 백필만 2분 14초.
#
# ⚠️ 이게 빠져 있어서 수급이 2026-08-25 에 멈춘 채 9거래일을 돌았다. 8/24~25 전체
#    수집에서 7종목이 속도제한 소진으로 실패했고, 그 뒤로 flow 를 아무도 안 돌렸다.
#    build_features 는 한 종목이라도 비면 **수급 피처를 통째로 건너뛴다**.
#
# 실패해도 멈추지 않는다 — 수급은 보조 피처고, 일봉·피처 갱신이 인질로 잡히면 안 된다.
echo "--- 수급 수집 ---"
"$PY" scripts/collect.py --tr flow --skip-index $END_ARG \
    || echo "수급 일부/전부 실패 — 다음 실행이 메운다. 피처는 계속 만든다"
