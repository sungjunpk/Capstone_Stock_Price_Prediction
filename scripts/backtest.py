...
def _print_diagnostics(result) -> None:
    ...
    if abs(ic["t_stat"]) < 2:
        ...
        # 유의한데 **음수**다. 이걸 "순위는 맞힌다"로 적으면 정반대로 읽힌다 —
        # 실제로 2026-09-09 idxrel 트랙(IC -0.0244, t=-3.70)에서 그렇게 찍혔다.
        # 뒤집으면 알파처럼 보이지만, 부호가 국면마다 바뀌면 그건 알파가 아니다.
        verdict = ("⚠️ 유의하게 **음의** IC — 상위로 고른 종목이 오히려 못 간다. "
                   "부호가 구간마다 안정적인지 먼저 확인할 것(뒤집기 전에)")
