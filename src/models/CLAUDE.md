...
**구현 완료 (2026-08-25).** **0.33M 파라미터**(`minimal` 프리셋) — 1.88M 으로
시작했다가 스윕에서 즉시 과적합해 축소 확정했다. 근거는
[`src/training/CLAUDE.md`](../training/CLAUDE.md) 의 스윕 표.
확정된 설계 판단 두 가지:
...
기본 초기화로 두면 `softplus(0)=0.693` 이라 초기 분위 간격이 0.69 인데,
5일 수익률의 전체 분위 폭은 0.124 다. **출력이 타깃보다 9배 크게 시작**한다.

실측(2026-08-25, 146종목 전체 학습): 이 상태로는 val pinball 0.013993 이 나왔는데
무조건부 분위수 기준선이 0.013285 였다 — **아무것도 안 하는 것보다 5.3% 나빴다.**
모델이 신호를 배우기 전에 스케일 줄이기에 학습 예산을 다 썼다.

수정: 학습 데이터의 무조건부 분위수를 `init_quantiles` 로 넣어 **기준선에서 출발**시킨다.
- 마지막 층 bias = 기준선 분위수(증분은 inverse-softplus)
- 마지막 층 weight = `std 1e-4`. **정확히 0으로 두면 안 된다** —
  `d(out)/d(pooled) = W = 0` 이라 인코더까지 경사가 전혀 안 간다(실측 확인).

## scale_target 은 기본 off

윈도우 변동성을 출력에 곱하면 상수(무조건부 분위수)조차 표현하기 어려워진다 —
헤드가 `1/scale` 을 학습해야 하기 때문이다. 기준선을 하한으로 깔고 가는 설계와 충돌한다.

## 모든 실험은 기준선과 비교한다

`train.py` 가 무조건부 분위수의 val 손실을 함께 재서 리포트에 남긴다.
**이걸 못 이기면 하이퍼파라미터가 아니라 피처/타깃 설계를 봐야 한다.**

## 데이터 규모 참고

학습샘플 **352,876개**(146종목, lookback 120일). 행 수는 늘었지만 **실질 독립
사건은 거래일 수(약 1,900일) 수준**이다 — 같은 날 146종목은 시장 공통 요인으로
묶인다. 이게 모델 크기를 정한 근거다.

현재 config 값은 `d_model=32`, `n_layers=1`, `dropout=0.5` 다(= `minimal`).
1.88M 짜리 `d_model=128 / n_layers=3 / dropout=0.2` 는 **폐기된 설정**이다 —
학습 로그에 `파라미터 1.88M` 이 뜨면 낡은 코드다
([`docs/KAGGLE_SETUP.md`](../../docs/KAGGLE_SETUP.md) 의 방어 참조).
