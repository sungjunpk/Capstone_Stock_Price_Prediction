...
## Anti-overfitting — 샘플 수에 속지 말 것

**명목 학습샘플 26만은 독립 정보가 아니다.**

| | |
|---|---|
| 인접 샘플 입력 겹침 | **99.2%** (lookback 120일 중 119일 공유) |
| 타깃 겹침 | t+1~t+5 구간이 인접 샘플과 4/5 중복 |
| 비중복 샘플 근사 | 52,097개 (명목의 20%) |
| 실질 독립 사건 | 거래일 수(약 1,900일) 수준 — 같은 날 146종목은 시장 공통 요인으로 묶인다 |

파라미터 1.88M 은 여기에 과하다. 실측(2026-08-25 전체 학습):
epoch 1(warmup 중 lr 6e-5)이 최고였고 **2 epoch째부터 val 이 기준선보다 나빠졌다.**
train 은 0.0132 → 0.0110 으로 계속 내려갔다 — 전형적인 즉시 과적합.

그래서:
- dropout / weight decay 를 세게 건다. **모델을 키우기 전에 정규화부터**
- **Val Loss 기준 early stopping** — train loss로 판단하지 않는다
- 설정 비교는 `scripts/sweep.py` 로 한 세션에서 돌린다(epoch당 약 53초)
- 승자의 best epoch 이 1이면 아직 과적합이다 — 더 줄인다

## 모든 실험은 기준선과 비교한다

`train.py` 가 무조건부 분위수의 val 손실을 함께 재서 리포트에 남긴다.
못 이기면 경고가 뜬다. **이걸 못 이기면 하이퍼파라미터가 아니라 피처/타깃 설계 문제다.**
