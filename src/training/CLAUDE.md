# src/training — 학습·분할·손실

> 루트 [`CLAUDE.md`](../../CLAUDE.md)의 절대 규칙이 우선한다.

## 분할 (leakage 방지 — 가장 중요)

- **날짜 기준 global train/val/test split 필수.** 종목별 분할이 아니다
- **정규화 통계는 train 구간에서만 계산한다.** val/test를 통계에 참여시키면 미래를 훔쳐본다
- 구간 경계에 **embargo**를 둔다. 타깃이 t+h를 보므로, embargo 없이 자르면
  train 마지막 샘플의 라벨이 val 구간을 훔쳐본다

현재 분할 (`configs/config.yaml`):

| 구간 | 기간 | 샘플 |
|---|---|---|
| train | ~2022-12-31 | 260,339 |
| val | ~2023-12-31 | 17,584 |
| test | 2024-01 ~ | 74,953 |

(146종목 기준. 8종목이던 시절 val이 968샘플이라 early stopping이 불안정할 우려가 있었는데,
유니버스 확대로 해소됐다.)

## 손실

Pinball(Quantile) Loss로 10/50/90 분위를 동시에 학습한다.

- `crossing_penalty`: 분위가 단조증가하지 않는 만큼 벌점. 이미 정렬돼 있으면 0
- 분위 교차를 방치하면 매매 신호의 기권 로직이 무너진다 (`src/trading/` 참고)

## Anti-overfitting — 샘플 수에 속지 말 것

**명목 학습샘플 26만은 독립 정보가 아니다.**

| | |
|---|---|
| 인접 샘플 입력 겹침 | **99.2%** (lookback 120일 중 119일 공유) |
| 타깃 겹침 | t+1~t+5 구간이 인접 샘플과 4/5 중복 |
| 비중복 샘플 근사 | 52,097개 (명목의 20%) |
| 실질 독립 사건 | 거래일 수(약 1,900일) 수준 — 같은 날 146종목은 시장 공통 요인으로 묶인다 |

파라미터 1.88M 은 여기에 과하다. 실측(2026-08-25 전체 학습):
epoch 1(warmup 중 lr 6e-5)이 최고였고 **2 epoch째부터 val 이 기준선보다 나빠졌다.**
train 은 0.0132 → 0.0110 으로 계속 내려갔다 — 전형적인 즉시 과적합.

그래서:
- dropout / weight decay 를 세게 건다. **모델을 키우기 전에 정규화부터**
- ⚠️ **단, 과적합이 늦춰졌다고 채택하지 않는다.** 2026-09-14 에 dropout 0.5 → 0.65 로
  best epoch 이 2 → 5 로 늦춰졌는데 **랭크 IC 가 0.0513 → 0.0010 으로 무너졌다.**
  모델이 방향을 포기하고 폭만 맞혀 val 이 좋아진 것이다. 정규화를 바꾸면
  **반드시 백테스트 랭크 IC 와 순열검정까지** 본다
- **Val Loss 기준 early stopping** — train loss로 판단하지 않는다
- 설정 비교는 `scripts/sweep.py` 로 한 세션에서 돌린다
- 승자의 best epoch 이 1이면 아직 과적합이다 — 더 줄인다

### 스윕 결과 (2026-08-25, 146종목 전체)

| 프리셋 | 파라미터 | best val | 기준선 대비 | best epoch |
|---|---|---|---|---|
| current | 1.88M | 0.014177 | +0.51% | 1 ← 과적합 |
| tiny | 0.49M | 0.013799 | +3.16% | 1 |
| small | 0.67M | 0.013782 | +3.28% | 1 |
| **minimal** | **0.33M** | **0.013772** | **+3.35%** | **3** ← 유일하게 과적합 없음 |

**작을수록 좋았다.** minimal 을 `configs/config.yaml` 기본값으로 확정했다.
아직 바닥을 못 찾았으므로 `nano`(0.24M) / `pico`(0.16M) 프리셋을 추가해두었다.

**통계적 유의성**: 개선폭 0.000478 을 날짜 단위 블록 부트스트랩으로 검정하면 **2.8σ**.
샘플 독립 가정으로는 5.5σ 가 나오지만 같은 날 146종목이 시장 공통 요인으로 묶이므로
과대평가다. 블록 부트스트랩 쪽이 맞는 수치다.

⚠️ val 이 2023년 한 해뿐이라 특정 국면에 치우쳤을 수 있다 —
walk-forward 백테스트로 여러 구간에서 재확인해야 한다.

## 모든 실험은 기준선과 비교한다

`train.py` 가 무조건부 분위수의 val 손실을 함께 재서 리포트에 남긴다.
못 이기면 경고가 뜬다. **이걸 못 이기면 하이퍼파라미터가 아니라 피처/타깃 설계 문제다.**

## Dataset 메모리 제약

학습샘플 35만 × lookback 120 × 피처 17 × 4바이트 = **2.9GB**.
윈도우를 미리 펼치면 터진다. 종목별 연속 배열은 28MB뿐이므로
`dataset.py` 는 배열을 한 번만 만들고 `__getitem__` 에서 구간을 잘라 준다.
이 계약은 `tests/test_dataset.py::test_no_window_duplication_in_memory` 가 지킨다.

종목마다 거래일이 다르다(공통 1,887 / 전체 2,857 — 거래정지 제거 때문).
공통 캘린더를 강요하면 34%가 날아가므로 **각 종목의 자기 행 위에서** 윈도우를 만든다.

## 학습 실행 위치

맥북(MPS)은 epoch당 약 19분(260k 샘플, batch 64)이라 전체 학습에 16시간이 걸린다.
학습은 외부 GPU로 옮긴다. **Kaggle 권장** — 주당 30시간, 세션 12시간이라 Colab 무료보다 길다.
절차: [`docs/KAGGLE_SETUP.md`](../../docs/KAGGLE_SETUP.md) /
노트북: `notebooks/kaggle_all_in_one.ipynb` (학습+백테스트 한 세션. 노트북은 이것 하나만 쓴다).
`train.py` 가 디바이스를 감지해 AMP/워커/pin_memory 를 자동 조정하므로
같은 명령이 양쪽에서 그대로 돈다.

## 워크플로 주의

학습 단계는 **결과를 직접 확인하면서** 진행한다.
자동 반복 실행으로 GPU/시간을 낭비하지 말 것 (루트 CLAUDE.md 워크플로 규칙).
