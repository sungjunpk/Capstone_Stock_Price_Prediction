...
| account_balance | kt00018 | 보유종목·매입단가 | ⬜ 미검증 — 주문 전 필수 |
| quote | ka10001 | 현재가 (주문 수량의 분모) | ✅ 검증완료(ka10001 검증분과 동일 TR) |
| buy_order | kt10000 | 매수 | ⬜ 미검증 |
| sell_order | kt10001 | 매도 | ⬜ 미검증 |
...
### account_balance (kt00018) — 확인할 것

- `qry_tp: '1'`(합산) / `dmst_stex_tp: 'KRX'`
- 보유목록이 `acnt_evlt_remn_indv_tot` 배열에 오는가
- **종목코드에 접두어가 붙는가** (`A005930`) — `broker._normalize_code` 가 벗기지만
  형태를 확인해 둘 것. 못 벗기면 보유분을 미보유로 착각해 **이력 버퍼가 무력화된다**
- `pur_pric`(매입단가)이 실제로 오는가 — **손절 기준가가 이 값이다**
- 요약(`tot_evlt_amt` 등)이 배열이 아니라 응답 최상위에 오는가

### 주문 TR (kt10000 / kt10001) — 확인할 것

- 필수 필드: `dmst_stex_tp('KRX')` `stk_cd` `ord_qty` `trde_tp`
- `trde_tp`: `'3'`=시장가(ord_uv 불필요), `'0'`=보통(지정가, ord_uv 필요)
- 응답의 주문번호 키가 `ord_no` 가 맞는가
