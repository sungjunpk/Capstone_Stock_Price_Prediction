...
17. **백테스트 구간 정렬 버그** — `run_backtest` 가 **예측이 시작되기 123일 전부터**
    수익률을 센다(가격은 2024-01-09~, 예측은 lookback 120 때문에 2024-07-03~).
    신호가 없어 강제로 현금인 기간이 연율화 분모에 들어가 지표를 깎는다.
    매수후보유는 그동안 투자돼 있으므로 **우리만 손해 보는 비교**다.
    정렬하면 같은 누적수익(56.1%)에 Sharpe 1.66→1.84, Calmar 2.61→3.27.
    ⚠️ 고치면 과거 리포트와 숫자가 달라지므로 한 번에 갈아야 한다
